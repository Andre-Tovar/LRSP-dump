...
def ESPPRC_CS(effs, DEMAND, VEHICLE_CAPACITY, N, CUSTOMER_SUBSET):
    class Vertex:
    ...
    class PartialPath:
        ...
        def extend(self, nextVertex, ew, CUSTOMER_SUBSET):
            ...
            for g in CUSTOMER_SUBSET: #range(1, N+1):
    ...
    # Initialization
    verticesDict = {}
    CUSTOMER_SUBSET.append(N)

    verticesDict[0] = Vertex(0, effs[0], DEMAND[0])
    for i in CUSTOMER_SUBSET: #range(N+1):
    ...
    verticesDict[0].add_partialPath(PartialPath(verticesDict[0], 0, 0, {i:0 for i in CUSTOMER_SUBSET}))
    ...
    while len(E) > 0:
        ...
        for exploringVertexIndex in CUSTOMER_SUBSET: #range(1,N+1):
