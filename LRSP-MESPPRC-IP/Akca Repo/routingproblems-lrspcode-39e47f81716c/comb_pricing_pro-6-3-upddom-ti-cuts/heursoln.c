//April 19 2006
//finds facility locations, assign customers, do clark and wright, and bin packing.
...
int Mod_DROP_HeurForFacilities (int num_REQ, int NFac, int NCust, double **d, int TIMELIMIT, int FCap, double *FCost, double VOcost,int *Demand,int ***customers, int **Selected){
  /*
  ...
  while (drop){
	...
	*/
	...
	if (feas[i]==1) {
	  for (j=0;j<NCust;j++){
	    if (assignment[i][j]!=-1) 
	      TCost[i]=TCost[i]+d[assignment[i][j]][i+NCust]*2;
	    else
	      break;
	  }
	}
...
double heurmain(double **d) {
  ...
  j=Mod_DROP_HeurForFacilities (num_REQ, NFac, NCust, d, TIMELIMIT, FCap, FCost, VOcost,Demand,&customers,&Selected);
