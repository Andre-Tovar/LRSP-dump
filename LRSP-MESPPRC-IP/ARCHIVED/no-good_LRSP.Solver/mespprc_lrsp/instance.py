...
class MESPPRCInstance:
    """
    Directed graph container for the LRSP pricing-oriented research prototype.
    ...
    In `mespprc_lrsp`, Phase 2 searches for a minimum reduced-cost feasible pairing of
    generated routes for one vehicle. Serving all customers is not required in pricing
    mode, so `required_customers` and `exact_once_service` are kept only for
    ...
    def required_customers(self) -> List[int]:
        ...
        The pricing solvers in `mespprc_lrsp` do not require full coverage of these
