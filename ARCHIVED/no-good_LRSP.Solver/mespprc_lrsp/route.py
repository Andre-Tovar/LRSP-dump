...
class Route:
    """
    Shared route object used across the two LRSP pricing phases.

    In `mespprc_lrsp`, `Route.cost` is interpreted as reduced cost. Routes are the
    feasible elementary single trips generated in Phase 1 and are later combined into a
    Phase 2 pairing, which represents a feasible vehicle schedule.

    The current package uses a single-facility/single-depot abstraction, so all routes
    are implicitly attached to the same facility.
    """
...
LRSPRoute = Route
LRSPRouteReductionRecord = RouteReductionRecord
