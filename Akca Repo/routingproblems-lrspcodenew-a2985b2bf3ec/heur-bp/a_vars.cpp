	...
	for (t=0; t < NFac; t++) {
	  ...
	  if (xstat[t] != FORBIDDEN) {
		...
		nonegcol=1; //no columns with neg. red. cost
	      ...
	      /* for debug */
	      //if ((counter == 109)&& (FAC == 2)) {
	      /*
	      printf("printing rc matrix: \n");
	      for (i=0; i < NCust+2; i++) {
		for (k=0; k < NCust+2; k++) 
		  printf("rc[%d][%d] = %lf \n", i,k,rc[i][k]);
	      }
	      */
	      // printf("red cost of x[3077]= %lf \n", lp_rc(3077));
	      //	printf("red cost of x[1694]= %lf \n", lp_rc(1694));
	      /*
	      column = Col_Array[FAC];
	      while (column){
		//if (column->Name == 509) {
		printf("lp_rc(y_%d_%d) = %lf \n", FAC, column->Name, lp_rc(column->varindex));
		for (i=0;i<NCust;i++){
		  if (column->sequence[i])
		    printf("%d:%d ", i, column->sequence[i]);
		}
		printf("\n");
		inq_var(column->varindex,YES);
		printf("obj coef = %lf, varindex = %d \n ", info_var.var_obj, column->varindex);
		//calc_rc = - info_var.var_obj;
		for (i=0; i<info_var.var_nz;i++) {
		  //calc_rc = calc_rc - info_var.var_coef[i]*(-lp_pi(info_var.var_ind[i]));
		  printf("const index = %d, coeff in const = %lf, dual of cons = %lf \n", info_var.var_ind[i],info_var.var_coef[i],lp_pi(info_var.var_ind[i]));
		}
		//printf("calculated red cost is %lf \n", calc_rc);
		//	break;
		//  }
		column = column->next;
	      }
...
// 		while (column){
// 		  if (column->Name == 417) {
// 		    printf("red cost of y_0_417 = %lf \n", lp_rc(column->varindex));
