//Generates feasible lrp solutions. Feb 27 2008
  ...
  */
  ...
  while (route) {
	...
	coltemp->labeldata.nodeseq[route->seq[i]]=i+1;
      ...
      // printf("d[%d][%d] = %lf \n",fac+NCust, route->seq[0],d[fac+NCust][route->seq[0]]);
      //for (i=1;i<route->size;i++){
      //	printf("d[%d][%d] = %lf \n",route->seq[i-1],route->seq[i],d[route->seq[i-1]][route->seq[i]]);
      //}
      //printf("d[%d][%d] = %lf \n",route->seq[route->size-1],fac+NCust,d[route->seq[route->size-1]][fac+NCust]);
  ...
  for (i=0;i<ncust;i++){
	...
	if (writelabellist){
	  ...
	  //	  printf("d[%d][%d] = %lf \n",fac+NCust,customers[i] ,d[fac+NCust][customers[i]]);
	  //printf("d[%d][%d] = %lf \n",customers[i],fac+NCust,d[customers[i]][fac+NCust]);
	}
  ...
  tcost=(double)numveh*VFcost+tvarcost;
...
double NearestNeighbor (double **d, int *customers, int *Demand, int TIMELIMIT, int VCap, int fac,int NCust, double VFcost, double VOcost,int *numcust, int *numcols, LABEL_NODE **ColList,int writelabellist){
  ...
  int *Visited;
  int i,p1,p2,index,count,unvisited,numveh,j;
  int Tdemand;
  int *RouteSeq;
  double tvarcost,tcost,*BinList, min, Tdist;
  ...
  while (unvisited==1) {
    ...
    for (i=0;i<NCust;i++){
      if ((Visited[i]==0)&&(d[p1][i]<=min-MyEPS)&&(Tdemand+Demand[i]<=VCap)) {
	index=i;
	min=d[p1][i];
      }
    }
  ...
  if (writelabellist){
    ...
    //printf("d[%d][%d] = %lf \n",fac+NCust,RouteSeq[0],d[fac+NCust][RouteSeq[0]]);
    //for (i=1;i<count;i++)
    // printf("d[%d][%d] = %lf \n",RouteSeq[i-1],RouteSeq[i],d[RouteSeq[i-1]][RouteSeq[i]]);
 ...
 /******************************************************************************/  
  ...
  for (i=0;i<NCust;i++){
    if (customers[i]!=-1){
      ...
      if ((Tdemand+Demand[tempindex]<=VCap)) { //feasible
...
double NearestInsertionHeuristic (double **d, int *customers, int *Demand, int TIMELIMIT, int VCap, int fac,int NCust, double VFcost, double VOcost, int *numcust, int *numcols, LABEL_NODE **ColList,int writelabellist){
  ...
  int *Visited;
  int i,p1,p2,indexcust, indexplace, count,unvisited,numveh,j;
  int Tdemand;
  double Tdist;
  int *RouteSeq, *route;
    ...
    for (i=0;i<NCust;i++){
	  ...
	  for (j=0;j<NCust+1;j++){
	    if ((route[j]!=-1)&&(route[j+1]!=-1)) {
	      if ((d[route[j]][i]+d[i][route[j+1]]-d[route[j]][route[j+1]]) <= min-MyEPS) {
		indexcust=i;
		indexplace=j+1;
...
int Mod_DROP_HeurForFacilities ( int NFac, int NCust, double **d, int TIMELIMIT, int FCap, double *FCost, double VOcost,int *Demand,int ***customers, int **Selected,int VCap, double VFcost,int heuristic,double *Xcoor, double *Ycoor){
  ...
  */
  ...
  while (drop){
	...
	TCost[i]=0;   //the cost when we close facility i
	for (j=0;j<NFac;j++) {
	  if ((i!=j)&&(Open[j]))
...
double Initial_Column_Generator(double **d,double *Xcoor, double *Ycoor, LABEL_NODE **ReturnInitCols,int *numcust, int *numcols, double closeness) {
  ...
  for (i=0;i<NFac;i++){
    ...
    for (j=0;j<NCust;j++)
      cust_bo_closeness[i][j]=j;
    cust_bo_closeness[i][NCust]=-1;
  }
  ...
  */
  ...
  for (j=0;j<NFac;j++) {
	...
	num_col=0;
	num_nz=0;
	x=SweepHeuristic (d,Xcoor,Ycoor,cust_bo_closeness[j], Demand, TIMELIMIT, VCap, j, NCust, VFcost, VOcost,&num_nz, &num_col, &ListofCols,1);
	//	printf("SWP Alg: fac= %d, numcols= %d, numNZ=%d \n",j,num_col,num_nz);
	numNZ[j]=numNZ[j]+num_nz;
	numCOL[j]=numCOL[j]+num_col;
      }
...
/* 
 ...
 //  printf("\n\n");
  ...
  for (j=0;j<NFac;j++) {
	...
	num_col=0;
	num_nz=0;
	x=NearestInsertionHeuristic (d,cust_bo_closeness[j], Demand, TIMELIMIT, VCap, j, NCust, VFcost, VOcost,&num_nz, &num_col, &ListofCols,1);
	//printf("NI Alg: fac= %d, numcols= %d, numNZ=%d \n",j,num_col,num_nz);
	numNZ[j]=numNZ[j]+num_nz;
	numCOL[j]=numCOL[j]+num_col;
      }
