...
{
  char buf[BUFSIZE],initnum_file[BUFSIZE],initcol_file[BUFSIZE];
  FILE *fp, *numF, *colF;
  int Nodes;
  int i,j,k,t;
  double f;
 ...
 */
 ...
 if (numselected){
   HeurFac=(int*)calloc (numselected,sizeof(int));
   HeurPairNum=(int*)calloc (numselected,sizeof(int));
   
   for (i=0;i<numselected;i++) {
     fscanf(numF, "%d %d \n", &HeurFac[i], &HeurPairNum[i]);
     printf("HeurFac[%d]=%d, HeurPairNum[%d]=%d \n",i,HeurFac[i],i,HeurPairNum[i]);
   }
   
   
 }
 else  
   printf("Num selected is zero!!!\n");
 ...
 *nzcnt = 0;
 ...
 if (numselected){
   for (i=0;i<numselected;i++){
     ...
     while (k<PairNum[fac]+HeurPairNum[i]){ 
       ...
       sprintf(&(_vstore[NAME_SIZE*(*vcnt)]), "Y_%d_%d", fac, k);
       _vstore[NAME_SIZE*(*vcnt + 1) - 1] = '\0';
       _vname[*vcnt] = &(_vstore[NAME_SIZE*(*vcnt)]);
       ...
       // printf("%d    %d      %d      %d  ",fac, column->Col_Time,cap,t);
       if(fac!=HeurFac[i]) {
	 printf("The data file can not be read correctly \n");
	 exit(9);
       }
       count=1;
       while (t!=-1){    
	 ...
	 if (column->sequence[t]){
       ...
       _vind[*nzcnt]=NCust+HeurFac[i];
       _vcoef[(*nzcnt)++]=  -(double) cap;
       
       for(t=0;t<NCust;t++){
	 if (column->sequence[t]){
	   _vind[*nzcnt]=NCust+NFac+HeurFac[i]*NCust+t;
	   _vcoef[(*nzcnt)++]= -(double) 1;
	 }
       }
       
       _vind[*nzcnt]=NCust+NFac+NCust*NFac+1+HeurFac[i];
       _vcoef[(*nzcnt)++]=  (double) 1;
       ...
       //****************************
       column->next=Col_Array[HeurFac[i]];
       Col_Array[HeurFac[i]]=column;
       //**************************
       
     }//while
     
   }//for all i, in numselected
 }
 else  
   printf("Num selected is zero!!!\n");
 ...
 if (numselected){
   for (i=0;i<numselected;i++){
     PairNum[HeurFac[i]]= PairNum[HeurFac[i]]+HeurPairNum[i];
   }
 }
  ...
  */
  ...
  //Calculate a heuristic UPPER BOUND    
